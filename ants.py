...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if hasattr(self.ant, 'is_Queen_a_bad_bitch'):
                if self.ant.is_Queen_a_bad_bitch:
                    return 
                else:
                    self.ant = None
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if hasattr(self.ant.ant, 'is_Queen_a_bad_bitch'):
                        if self.ant.ant.is_Queen_a_bad_bitch:
                            return 
                    self.ant.ant = None
                else:
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        return self.container and self.ant is None and ant.container is False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 3B
        nearestBee = self.place
        closestPlace = None 
        i = 0
        while nearestBee is not hive:
            if len(nearestBee.bees) != 0 and i <= self.max_range and i >= self.min_range:
                return random_or_none(nearestBee.bees)
            else:
                nearestBee = nearestBee.entrance
                i += 1
        return None
        # END Problem 3B
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            [bee.reduce_armor(self.damage) for bee in current_place.bees[:] if len(current_place.bees) > 0]
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6B
        nearbyBee = self.place
        if self.digesting == 0:
            if len(nearbyBee.bees) != 0:
                self.eat_bee(random_or_none(nearbyBee.bees))
                self.digesting = self.time_to_digest
        else:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.is_Queen_a_bad_bitch = QueenAnt.is_Queen_a_bad_bitch
        QueenAnt.is_Queen_a_bad_bitch = False
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.is_Queen_a_bad_bitch:
            ScubaThrower.action(self, colony)
            while location is not None:
                if location.ant is not None: 
                    if location.ant.container and location.ant.ant is not None:
                        if location.ant.ant not in self.doubled_ants:
                           location.ant.ant.damage *= 2
                           self.doubled_ants.append(location.ant.ant) 
                    if location.ant not in self.doubled_ants:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        super().reduce_armor(amount)
        if self.armor <= 0 and self.is_Queen_a_bad_bitch:
            bees_win()
